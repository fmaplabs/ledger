# Stage 13: Neovim plugin (ledger.nvim)

Requirement #2 from `docs/ledger.md`: the editor side of the heartbeat
...
cloud sync. It spawns `ledger heartbeat --file <path> [--write]`
...
hook. The statusline reads one new machine-readable command, `ledger
...
- [x] `ledger status [--json]` — identity from cwd (like `heartbeat`),
  ...
  for the consumer. Human mode prints `ledger · main · 2h 13m today`
- [x] `tests/common/mod.rs` — the temp-repo + `LEDGER_HOME` harness shared
  by the init/hook and status integration suites; `tests/integration_status.rs`
  pins the JSON contract field by field
- [x] `lua/ledger/cli.lua` — pcall-wrapped `vim.system` spawns
...
- [x] `lua/ledger/heartbeat.lua` — guard chain (cheapest first: buftype,
...
- [x] dots integration — unpack spec (`src = ".../ledger"`) + lualine
...
accepts an absolute path, and `:checkhealth ledger` diagnoses exactly
...
- [`:h health-dev`](https://neovim.io/doc/user/health.html#health-dev) — how
  `lua/ledger/health.lua` gets discovered by `:checkhealth`
